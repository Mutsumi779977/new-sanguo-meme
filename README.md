...
一个**模块化架构的梗工程系统**：收集 → 学习 → 输出。

> **好消息**是我已经完成了基本的梗库内容，只需要根据个人需求添加不同的梗就好。由于个人不可能遍历所有的梗，所以请**列位诸公**自行发挥。
> 
> **更好的消息**：由于我已经基本完成了梗库和输出模式的编写，列位诸公使用时大部分情况都是从本地梗库调取内容，所以**不用担心 Token 消耗**！
