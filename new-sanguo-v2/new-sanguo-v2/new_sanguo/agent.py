...
class NewSanguoAgent:
    ...
    def __init__(self, user_id: str = "default") -> None:
        ...
        # 加载底线梗配置和自我认知
        self.fallback_genkus = self._load_fallback_genkus()
        self.identity = self.fallback_genkus.get('identity', {}).get('full_intro', '我是新三国梗agent')
...
• 不死板硬套原文"""
    ...
    def _cmd_info(self, args: str) -> str:
        ...
        identity = getattr(self, 'identity', '我是新三国梗agent')
        return f"""🎭 {identity}
版本: v2.8.0
状态: {self.state.name}
...
meta梗库: {len(self.service.get_meta_genkus())} 条"""
    ...
    def _handle_chat(self, text: str) -> Union[str, Dict]:
        ...
        if confidence > high_confidence_threshold:
            topic_response = self._generate_topic_response(topic, text)
            if topic_response and not topic_response.startswith("📡"):
                return topic_response

        # 8. 完全无法匹配，使用底线梗作为最终兜底
    ...
    def _load_fallback_genkus(self) -> Dict:
        """加载底线梗配置"""
        try:
            import yaml
            fallback_path = Path(__file__).parent.parent / 'data' / 'fallback_genkus.yaml'
            if fallback_path.exists():
                with open(fallback_path, 'r', encoding='utf-8') as f:
                    return yaml.safe_load(f) or {}
            return {}
        except Exception as e:
            self.logger.warning(f"加载底线梗配置失败: {e}")
            return {}
    
    def _generate_fallback_response(self, text: str) -> str:
        """
        生成底线梗回复（v2.8.4 随机选择+回退机制）
        ...
        2. 如果需要核心词：
           - 尝试提取并填充
           - 成功则输出
           - 失败则标记为不可用，重新随机选择（避免死循环）
        3. 如果不需要核心词，直接输出
        4. 如果所有有变量的梗都失败，强制使用无变量梗
        
        Returns:
            底线梗回复文本
        """
        if not self.fallback_genkus:
            return "📡 暂无匹配，教教我？发送 /录入"
        
        genkus = self.fallback_genkus.get('fallback_genkus', [])
        if not genkus:
            return "📡 暂无匹配，教教我？发送 /录入"
        
        # 预提取关键词（只做一次）
        keywords = self._extract_keywords_for_fallback(text)
        k1, k2 = keywords[0], keywords[1] if len(keywords) > 1 else keywords[0]
        content = keywords[2] if len(keywords) > 2 else (keywords[0] if keywords else text[:2])
        has_valid_keywords = keywords[3] if len(keywords) > 3 else True
        
        # 分类：需要变量的 vs 不需要变量的
        variable_types = {'unknown_people', 'nonsense', 'teach_fool', 'threaten_leave'}
        no_variable_types = {'drive_away', 'angry_reject'}
        
        variable_genkus = [g for g in genkus if g.get('type') in variable_types]
        no_variable_genkus = [g for g in genkus if g.get('type') in no_variable_types]
        
        # 记录已尝试的梗（避免死循环）
        tried_ids = set()
        max_attempts = len(genkus) + 5  # 安全上限
        attempts = 0
        ...
        while attempts < max_attempts:
            ...
            # 剩余可用的梗
            available = [g for g in genkus if g.get('id') not in tried_ids]
            if not available:
                break  # 全部尝试过了
            
            # 按权重随机选择一个
            weights = [g.get('weight', 1) for g in available]
            selected = random.choices(available, weights=weights, k=1)[0]
            tried_ids.add(selected.get('id'))
            
            template = selected.get('template', '')
            genku_type = selected.get('type', 'unknown')
            
            # 判断是否需要变量
            needs_variable = genku_type in variable_types
            ...
            if has_valid_keywords:
                ...
                try:
                    if genku_type == 'unknown_people':
                        return template.format(keyword1=k1, keyword2=k2)
                    else:  # nonsense, teach_fool, threaten_leave
                        return template.format(content=content)
                except (KeyError, IndexError, ValueError):
                    # 填充失败，继续尝试下一个
                    continue
            else:
                # 没有有效关键词，无法填充，继续尝试下一个
                continue
        
        # 所有尝试都失败，强制使用无变量梗（随机选一个）
        if no_variable_genkus:
            weights = [g.get('weight', 1) for g in no_variable_genkus]
            selected = random.choices(no_variable_genkus, weights=weights, k=1)[0]
            return selected.get('template', '叉出去！')
        
        # 终极兜底
        return "叉出去！"
    
    def _extract_keywords_for_fallback(self, text: str) -> List[str]:
        """
        为底线梗提取关键词（v2.8.3 修订版）
        
        规则：
        - 对于 unknown_people 类型（k1,k2）：各取核心词的一个字
          例如：核心词"神人" -> "神", "人"
          如果只有一个核心词，取它的前两个字
        - 对于 nonsense/threaten_leave/teach_fool 类型（content）：取一个核心词（2-4字）
          例如："兵法", "算法"
        
        返回：[k1, k2, content, is_valid]
          is_valid: 是否提取到了有效关键词（True/False）
        """
        # 停用词（虚词、代词、量词等）
        stop_chars = {'的', '了', '是', '在', '我', '你', '他', '她', '它', 
                      ...
                      '对', '将', '向', '有', '没', '会', '能', '要', '去', 
                      '来', '上', '下', '里', '外', '很', '太', '非常', '也',
                      '什', '怎'}  # 什(什么)、怎(怎么)通常不单独使用
        
        # 弱词（常见但不适合做核心的词）
        weak_words = {'什么', '怎么', '为什么', '如何', '怎样', '东西', '事情', 
                      '问题', '情况', '时候', '地方', '样子', '这个', '那个',
                      '这些', '那些', '这里', '那里', '这么', '那么', '什么什么',
                      '你的', '我的', '他的', '她的', '它的', '就是', '还是'}
        
        # 提取英文单词（2个字母以上）- 优先处理，避免被中文拆分干扰
        english_words = re.findall(r'[a-zA-Z]{2,}', text)
        
        # 提取所有中文字符序列（连续的中文作为一个词）
        ...
        for word in english_words:
            if word.lower() not in {'is', 'am', 'are', 'was', 'were', 'be', 'been'}:
        ...
        for word in chinese_words:
            if word in weak_words:
                continue
            # 跳过以指示代词或停用字开头的词
            if word[0] in {'这', '那', '我', '你', '他', '她', '它', '怎', '的', '个', '是', '了'}:
                continue
            meaningful_words.append(word)
        
        # 标记是否提取到了有效关键词
        has_valid_keywords = len(meaningful_words) > 0
        
        # 如果过滤后没有词，尝试从原文提取（fallback模式）
        if not meaningful_words:
            # 提取所有中文字符
            chinese_chars = re.findall(r'[\u4e00-\u9fa5]', text)
            # 排除停用字后组合
            filtered_chars = [c for c in chinese_chars if c not in stop_chars]
            if len(filtered_chars) >= 2:
                meaningful_words = [filtered_chars[0] + filtered_chars[1]]
                # 检查fallback提取的词是否仍有意义
                has_valid_keywords = True
            elif chinese_chars:
                meaningful_words = [chinese_chars[0] + (chinese_chars[1] if len(chinese_chars) > 1 else '某')]
                has_valid_keywords = False  # 全是停用字，标记为无效
            elif english_words:
                meaningful_words = english_words
                has_valid_keywords = True
            else:
                meaningful_words = ['未知']
        ...
        if len(meaningful_words) >= 2:
            # 从两个不同的词中取第一个字
            word1 = meaningful_words[0]
            word2 = meaningful_words[1]
            # 英文词取首字母，中文取第一个字
            k1 = word1[0].upper() if word1.isalpha() and word1[0].isascii() else word1[0]
            k2 = word2[0].upper() if word2.isalpha() and word2[0].isascii() else word2[0]
            result = [k1, k2]
        else:
            # 只有一个词，取前两个字
            word = meaningful_words[0]
            if len(word) >= 2:
                k1 = word[0].upper() if word[0].isalpha() and word[0].isascii() else word[0]
                k2 = word[1].upper() if word[1].isalpha() and word[1].isascii() else word[1]
                result = [k1, k2]
            else:
                k1 = word[0].upper() if word[0].isalpha() and word[0].isascii() else word[0]
                result = [k1, '某']
        
        # ===== 提取 content（一个核心词，2-4字）=====
        content_word = None
        ...
        if not content_word:
            ...
            content_word = first_word[:4] if len(first_word) > 4 else first_word
        
        result.append(content_word)
        
        # 添加有效性标志
        result.append(has_valid_keywords)
