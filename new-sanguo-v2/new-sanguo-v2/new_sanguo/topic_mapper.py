...
class TopicMapper:
    ...
    TOPIC_PATTERNS = {
        TopicCategory.ESPORTS: {
            ...
            'emotions': ['震惊', '不服输', '愤怒', '遗憾', '赞赏', '得意'],
            'scenes': ['竞争', '对决', '胜负', '大胜', '惜败'],
        },
        TopicCategory.TECH: {
            'keywords': [
                'ai', 'gpt', 'claude', 'kimi', '大模型', '芯片', '显卡', 'cpu',
                '华为', '苹果', '小米', '发布会', '新品', '科技', '数码',
                'gpt4', 'llm', '算力', '算法', 'agent', '智能体'
            ],
            'emotions': ['震惊', '自负', '自信', '嘲讽', '赞赏'],
            'scenes': ['创新', '突破', '比较', '发布', '更新'],
        },
        TopicCategory.ENTERTAINMENT: {
            ...
            'emotions': ['嘲讽', '调侃', '兴奋', '失望', '震惊'],
            'scenes': ['评价', '比较', '吐槽', '版权', '纠纷'],
        },
        ...
        TopicCategory.POLITICS: {
            ...
            'emotions': ['震惊', '愤怒', '嘲讽', '难以置信', '讽刺'],
            'scenes': ['战争', '冲突', '国际', '外交', '威胁', '退缩'],
        },
        TopicCategory.GAME: {
            'keywords': [
            ...
            'emotions': ['愤怒', '无奈', '吐槽', '自嘲', '质疑', '恍然大悟'],
            'scenes': ['游戏', '失败', '抱怨', '抽卡', '歪了', '保底', '反转'],
        },
        # 新增三类（从V2吸收）
        TopicCategory.FOOD: {
            'keywords': [
                '火锅', '川菜', '粤菜', '湘菜', '鲁菜', '东北菜', '烧烤', '烤肉', '日料', '寿司',
                '拉面', '麻辣烫', '奶茶', '咖啡', '好吃', '难吃', '餐厅', '饭店', '美食',
                '料理', '米其林', '必吃', '推荐菜', '招牌菜', '外卖', '排队', '等位'
            ],
            'emotions': ['满足', '享受', '挑剔', '吐槽', '推荐'],
            'scenes': ['聚餐', '探店', '吐槽', '推荐', '吃饭'],
        },
        TopicCategory.MOVIE: {
            'keywords': [
                '电影', '票房', '豆瓣', '影评', '上映', '首映', '点映', '撤档', '重映',
                '诺兰', '导演', '演员', '主演', '配角', '彩蛋', 'IMAX', 'imax', '杜比',
                '院线', '影院', '电影院', '爆米花', '春节档', '暑期档', '国庆档'
            ],
            'emotions': ['震撼', '失望', '推荐', '吐槽', '感动', '无聊'],
            'scenes': ['观影', '评价', '剧透', '讨论', '档期'],
        },
        TopicCategory.ANIME: {
            'keywords': [
                '动漫', '番剧', '动画', '二次元', '追番', '新番', '番', '季', 'ova', '剧场版',
                '火影', '海贼王', '柯南', '进击的巨人', '鬼灭', '咒术', 'jojo', 'JOJO',
                'b站', 'B站', '哔哩哔哩', 'bilibili', '弹幕', 'op', 'ed', 'ed', 'op',
                '声优', '配音', '漫改', '轻改'
            ],
            'emotions': ['感动', '燃', '治愈', '催泪', '吐槽'],
            'scenes': ['追番', '补番', '讨论', '二创', '漫展'],
        },
        TopicCategory.DAILY: {
            'keywords': [
                '吃', '晚饭', '午饭', '早餐', '吃什么', '喝什么',
            ...
            'emotions': ['轻松', '随意', '调侃', '无奈', '疲惫', '自嘲', '客气'],
            'scenes': ['闲聊', '日常', '无厘头', '吃饭', '天气', '状态', '招呼', '告别'],
        }
    }
    ...
    TOPIC_KNOWLEDGE_REQUIREMENTS = {
        ...
        TopicCategory.FOOD: {
            'needs_context': False,
            'keywords_that_need_search': [],
            'local_knowledge_keywords': ['好吃', '难吃', '排队', '推荐'],
        },
        TopicCategory.MOVIE: {
            'needs_context': True,
            'keywords_that_need_search': ['最新', '怎么样', '好看吗', '票房', '评分'],
            'local_knowledge_keywords': ['豆瓣', '影评', '上映', '彩蛋'],
        },
        TopicCategory.ANIME: {
            'needs_context': False,
            'keywords_that_need_search': [],
            'local_knowledge_keywords': ['追番', '补番', '新番', '完结'],
        },
        TopicCategory.DAILY: {
    ...
    def _build_keyword_index(self):
        ...
        # 定义需要精确匹配的短关键词（避免子串误匹配）
        self.exact_match_keywords = {'gen', 'ai', 'bo5'}
        
    def _is_exact_match(self, keyword: str, text: str) -> bool:
        """检查是否是精确匹配（单词边界或独立出现）"""
        if keyword not in self.exact_match_keywords:
            return True  # 非精确匹配关键词，使用普通子串匹配
        

        # 使用单词边界匹配短关键词
        pattern = r'(?:^|[\s\b])' + re.escape(keyword) + r'(?:[\s\b]|$|[：:]\d)'
        ...
        for topic, config in self.TOPIC_PATTERNS.items():
            ...
            for kw in config['keywords']:
                ...
                if kw_lower in text_lower:
                    # 对于短关键词，使用精确匹配检查
                    if not self._is_exact_match(kw_lower, text_lower):
                        continue  # 跳过子串误匹配
                    score += 1
                    # 完全匹配加分（单词边界内）
                    if kw_lower == text_lower or f' {kw_lower} ' in f' {text_lower} ':
                        score += 1
            if score > 0:
                scores[topic] = score
    ...
    def get_mapping(self, topic: TopicCategory) -> Dict:
        ...
        config = self.TOPIC_PATTERNS.get(topic, {
            'emotions': [],
            'scenes': [],
        })
        # 只返回情绪和场景，不返回人物（人物由梗库标签决定）
        return {
    ...
    def suggest_genku_tags(self, text: str) -> Dict[str, List[str]]:
        ...
        # 新增三类处理（从V2吸收）
        elif topic == TopicCategory.FOOD:
            # 美食话题 - 典型场景：品尝后评价
            suggestions['emotions'] = ['满足', '享受', '挑剔']
            suggestions['scenes'] = ['聚餐', '探店', '评价']
            # 可以用"好酒，真是好酒"或"公台"等通用好评/感叹模板
            suggestions['context_template'] = '好酒，真是好酒'
        
        elif topic == TopicCategory.MOVIE:
            # 电影话题 - 典型场景：观影后评价
            if '烂' in text or '难看' in text or '浪费' in text:
                suggestions['emotions'] = ['失望', '吐槽', '愤怒']
                suggestions['scenes'] = ['评价', '吐槽']
                suggestions['context_template'] = '差点没把我笑死'
            else:
                suggestions['emotions'] = ['震撼', '推荐', '赞赏']
                suggestions['scenes'] = ['观影', '评价']
                suggestions['context_template'] = '我原本以为{对象}已经天下无敌了'
        
        elif topic == TopicCategory.ANIME:
            # 动漫话题 - 典型场景：追番/完结讨论
            if '完结' in text or '结局' in text or '烂尾' in text:
                suggestions['emotions'] = ['失望', '遗憾', '感慨']
                suggestions['scenes'] = ['完结', '讨论']
            else:
                suggestions['emotions'] = ['燃', '感动', '治愈']
                suggestions['scenes'] = ['追番', '补番']
...
def get_topic_mapper(reset: bool = False) -> TopicMapper:
    """
    获取话题映射器实例
    ...
    if reset or _topic_mapper is None:
        _topic_mapper = TopicMapper()
    return _topic_mapper


def reset_topic_mapper():
    """重置话题映射器单例（测试用）"""
    global _topic_mapper
    _topic_mapper = None
