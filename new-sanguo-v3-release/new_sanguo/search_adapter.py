...
class SearchAdapter:
    ...
    def _fallback_search(self, query: str) -> Optional[SearchResult]:
        """
        备用搜索方案
        ...
        return SearchResult(
            query=query,
            summary=f"从查询中提取的关键词: {', '.join(keywords[:5])}",
            keywords=keywords,
            raw_results=[]
        )
...
def get_search_adapter(enabled: bool = True, result_count: int = 3) -> SearchAdapter:
    """获取搜索适配器实例（单例）"""
